...
static PerimeterExtrusions extract_ordered_perimeter_extrusions(
    ...
    for (const PerimeterExtrusion &perimeter_extrusion : sorted_perimeter_extrusions) {
        ...
        // extrusion.inset_idx (physical wall slot) is used rather than depth (graph distance) so
        // that the loop repositioned here is exactly the one carrying the FirstInternalPerimeter
        // role and its associated speed/acceleration settings. In thin or branching regions
        // depth != inset_idx, and using depth would reorder a loop that does not have that role.
        //
